...
function obtenerAsignadosAsesor(nombreAsesor) {
  try {
    ...
    // Filtrar por asesor y que NO esté gestionado
    const asignados = data.slice(1)
      ...
      .map(r => ({
        ...
        fechaRaw: r[1] instanceof Date ? r[1].getTime() : new Date(r[1]).getTime()
      }));
...
function claimNextCase(agentName) {
  ...
  try {
    ...
    // PRIORIDAD 1: Recopilar casos ya asignados a este asesor pero no gestionados
    // y ordenar por fecha ASCENDENTE (más antiguo primero)
    const asignadosPendientes = [];
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      if (String(row[0]).trim() === agentName && !String(row[9]).trim() && String(row[2]).trim() !== "") {
        const fechaRaw = row[1] instanceof Date ? row[1].getTime() : new Date(row[1]).getTime();
        asignadosPendientes.push({ rowIndex: i, fechaRaw: fechaRaw });
      }
    }

    if (asignadosPendientes.length > 0) {
      // Ordenar por fecha ascendente (más antiguo primero)
      asignadosPendientes.sort((a, b) => a.fechaRaw - b.fechaRaw);

      const i = asignadosPendientes[0].rowIndex;
    ...
    // PRIORIDAD 2: Recopilar todos los casos libres y ordenar por fecha ASCENDENTE
    const casosLibres = [];
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      if (!String(row[0]).trim() && !String(row[9]).trim() && String(row[2]).trim() !== "") {
        const fechaRaw = row[1] instanceof Date ? row[1].getTime() : new Date(row[1]).getTime();
        casosLibres.push({ rowIndex: i, fechaRaw: fechaRaw });
      }
    }

    if (casosLibres.length === 0) {
      return { success: false, message: 'No hay radicados pendientes de gestión en este momento.' };
    }

    // Ordenar por fecha ascendente (más antiguo primero)
    casosLibres.sort((a, b) => a.fechaRaw - b.fechaRaw);

    const i = casosLibres[0].rowIndex;
...
function saveManagement(data) {
  try {
    ...
    // Mapeo de columnas: J=10, K=11, L=12, M=13, N=14, O=15
